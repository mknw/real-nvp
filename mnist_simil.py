...
def main(args, model_meta_stuff = None):
    ...
    if os.path.isfile(stats_filename) and not args.force:
        ...
        loss_fn = RealNVPLoss()
        # stats = track_distribution_mnist(net, testloader, device, loss_fn)
        stats = track_z_mnist(net, testloader, device, loss_fn)
...
def PCA_eig_np(Z, k, center=True, scale=False):
    ...
    return {'z': Z, 'k': k, 'components': components.T,
                'var_exp': explained_variance}

def PCA_test(z_s, k, center=False, mode='self'):
    # if not isinstance(k, int) and k is not 'mle' and not isinstance(k, float):
    # 	raise ValueError
    ...
    if mode == 'sklearn':
        ...
        n_pcs = len(PCs.explained_variance_)
...
def plot_reduced_dataset(pca, z_s, k, filename):
...
def plot_PCA(pca, z_s, k, filename):
    # sort from highest variance
    if isinstance(pca, PCA):
        components = pca.components_[::-1]
        var_exp = pca.explained_variance_[::-1]
        z_s, y = label_zs(z_s)
...
def track_z_mnist(net, loader, device, loss_fn, **kwargs):
